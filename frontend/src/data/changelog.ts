...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      { title: 'Customizable transitions', detail: 'Customize ▸ Editor now has a Transitions list: click “Add Transition” to enter your own (they appear as you type in a Transition element), drag to reorder, and hide built-ins you never use. “WIPE TO:” is now a built-in. Built-ins can be hidden but not deleted; your own can be removed.', tags: ['New Feature', 'Editor'] },
      { title: 'New “Editor” tab in Customize', detail: 'The Customize “Elements” tab is now “Editor” and sits at the top of the list (it covers elements and transitions). Customize now opens on this tab by default, the window opens lower so it no longer overlaps the ribbon, and Side Panels moved above Quick Access.', tags: ['UI'] },
      ...
      { title: 'Ribbon editor sizing + divider visibility', detail: 'Sections in the toolbar editor no longer render larger than they do on the saved bar — they start compact and grow only as items are added (the section title field no longer forces them wide). Inline dividers now use the same color and weight as full-height section dividers, so they’re no longer hard to see.', tags: ['Fix', 'UI'] },
      { title: 'Typewriter tool renamed “Focus”', detail: 'The Typewriter tool is now called “Focus”.', tags: ['UI', 'Tools'] },
      { title: 'Backup & Restore settings', detail: 'Settings ▸ General ▸ Backup & Restore exports all your settings and customizations (design, toolbar/ribbon layout, themes, elements, transitions, shortcuts, menu system…) to a file, and imports them into another install — so a release build can inherit everything from the dev app. Sign-in and device tokens are left out for safety. Scripts aren\'t affected.', tags: ['New Feature'] },
      { title: 'Native menus by default on macOS', detail: 'A fresh install (including a release build, whose storage is separate from the dev app) now defaults to the native macOS menu bar instead of the in-window one. An explicit choice in Settings still wins.', tags: ['Fix', 'UI'] },
      { title: 'Updated app logo', detail: 'Swapped in the refined “SC” mark (smaller letters) everywhere the old one lived — the macOS/Windows/iOS/Android app icons, the browser favicon, the loading splash and the About window. The first-run welcome screen also showed the old “OD” monogram; it now shows “SC”.', tags: ['Branding'] },
      { title: 'Focus centers the last line of the script', detail: 'At the end of the script there was no room below to keep the current line centered, so it drifted low. The page now gets enough room below the last page — plus half a page of extra scroll — and the line re-centers after pagination reflows, so it stays put right to the final line.', tags: ['Fix', 'Tools'] },
      { title: 'Focus tool: one list', detail: 'The Focus tool no longer splits its options under “Scrolling” and “Focus” headings — everything sits together in one list.', tags: ['UI', 'Tools'] },
      { title: 'Focus highlight bar fits the page in Extreme focus', detail: 'With “Highlight the current line” and “Extreme focus” both on, the highlight bar was stuck to the left after the layout resized. It now follows the page whenever the window, panels or fullscreen change.', tags: ['Fix', 'Tools'] },
    ],
  },
  {
    version: '4.21',
  ...
  },
  {
    ...
    date: '2026-07-19',
    items: [
      { title: 'Small ribbon buttons pulled noticeably tighter', detail: 'v4.6\'s trim was too subtle to see. The small (two-row) icon boxes are narrower again (compact 26→20px, comfortable 28→22px), the row gap is pinned to 1px, and the wrapper\'s internal padding is gone — so the icons now read as a tight group. Big buttons are unchanged.', tags: ['UI', 'Polish'] },
    ],
  },
  {
    version: '4.5',
  ...
  },
  {
    ...
    date: '2026-07-19',
    items: [
      { title: 'Page Zoom +/- icons', detail: 'The +/- icons are 12px and sit right next to the % number.', tags: ['UI'] },
    ],
  },
  {
    version: '3.91',
