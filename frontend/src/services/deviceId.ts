...
function detectPlatform(): string {
  try {
    ...
    if (isTauri) {
      if (/Android/i.test(ua)) return 'Android (FreeDraft)';
      if (/iPhone|iPad|iPod/i.test(ua)) return 'iOS (FreeDraft)';
      if (/Macintosh|Mac OS X/i.test(ua)) return 'macOS (FreeDraft)';
      if (/Windows/i.test(ua)) return 'Windows (FreeDraft)';
      if (/Linux/i.test(ua)) return 'Linux (FreeDraft)';
      return 'Desktop (FreeDraft)';
    }
