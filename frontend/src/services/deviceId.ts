...
function detectPlatform(): string {
  try {
    ...
    if (isTauri) {
      if (/Android/i.test(ua)) return 'Android (ScriptCraft)';
      if (/iPhone|iPad|iPod/i.test(ua)) return 'iOS (ScriptCraft)';
      if (/Macintosh|Mac OS X/i.test(ua)) return 'macOS (ScriptCraft)';
      if (/Windows/i.test(ua)) return 'Windows (ScriptCraft)';
      if (/Linux/i.test(ua)) return 'Linux (ScriptCraft)';
      return 'Desktop (ScriptCraft)';
    }
