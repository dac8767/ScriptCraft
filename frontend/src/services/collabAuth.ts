...
async function backendAuthRequest<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  if (!base) {
    throw new Error('FreeDraft Cloud is not configured for this app. Open Settings → System Settings to set the FreeDraft server URL.');
  }
  ...
  try {
    ...
    throw wrapNetworkError(err, 'the FreeDraft backend');
  }
